...
glmnet2 = cv.glmnet(mmatrix2,input2$Bolt_1_Tensile[subset1], type.measure="mae")
...
formulastring2 = paste("Bolt_3_Tensile  ~ ", paste(covariates, collapse="*"), "+one_div_last_start * (", paste(covariates[1:6], collapse="+"), ")")
#formulastring2 = paste("Bolt_3_Tensile  ~ ", paste(covariates, collapse="*"), "+one_div_last_start * timepoints" )
#covariates = c(names(input2)[c(1:2, 4:8,22)])
#formulastring2 = paste("Bolt_1_Tensile  ~ ", paste(covariates, collapse="*"))
formula2 = formula(formulastring2)
#training linear model on training data using all possible interactions
...
#predict on holdout set:
predlm2 = predict(lm2, input2[!subset1,])
mpe_lm2 = mpe(predlm2, input2$Bolt_3_Tensile[!subset1] )
mpe_lm2
ll = length(input2$Bolt_3_Tensile[!subset1])
mpe(predlm2[round(ll/2):ll], (input2$Bolt_3_Tensile[!subset1] )[round(ll/2):ll])

plot(predlm2,type="l",col=2)
lines(input2$Bolt_3_Tensile[!subset1], col=1)
...
plot(predict(glmnet2,model.matrix(formula2, data= test)))
